...
class PrimeraProtectionPolicyInputSchema(BaseModel):
    """
    ...
    non_zero_rto_config: Optional[StrictStr] = Field(default=None, description="Non-Zero RTO configuration to be used - Active Sync. Even the plain synchronous replication is non zero RTO configuration but for this plain sync configuration, no need to specify any value in this parameter. Cannot be used along with zeroRtoConfig", alias="nonZeroRtoConfig")
    ...
    remote: ProtectionPolicyRemoteInputSchema = Field(description="Replication partner details for remote protection")
    ...
    secondary_remote: Optional[ProtectionPolicySecondaryRemoteInputSchema] = Field(default=None, description="Replication partner details for Async partner in Synchronous Long Distance mode and for 3DC Peer Persistence mode", alias="secondaryRemote")
    zero_rto_config: Optional[StrictStr] = Field(default=None, description="Zero RTO configuration to be used. Peer Persistence, Active Peer Persistence. Cannot be used along with nonZeroRtoConfig", alias="zeroRtoConfig")
    __properties: ClassVar[List[str]] = ["autoRecover", "autoSynchronize", "noAutomaticSynchronization", "nonZeroRtoConfig", "overPeriodAlert", "remote", "rpoSecs", "secondaryRemote", "zeroRtoConfig"]

    @field_validator('non_zero_rto_config')
    def non_zero_rto_config_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ActiveSync']):
            raise ValueError("must be one of enum values ('ActiveSync')")
        return value
    ...
    def zero_rto_config_validate_enum(cls, value):
        ...
        if value not in set(['PP', 'APP']):
            raise ValueError("must be one of enum values ('PP', 'APP')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "remote": ProtectionPolicyRemoteInputSchema.from_dict(obj["remote"]) if obj.get("remote") is not None else None,
            "rpoSecs": obj.get("rpoSecs"),
            "secondaryRemote": ProtectionPolicySecondaryRemoteInputSchema.from_dict(obj["secondaryRemote"]) if obj.get("secondaryRemote") is not None else None,
