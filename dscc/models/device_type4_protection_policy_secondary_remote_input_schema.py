...
class DeviceType4ProtectionPolicySecondaryRemoteInputSchema(BaseModel):
    """
    ...
    replication_type: StrictStr = Field(description="Replication type. Supported type is periodic only. Periodic replication (Asynchronous protection policy) provides protection from array or site failure with the user defined RPO.", alias="replicationType")
    __properties: ClassVar[List[str]] = ["partnerId", "partnerName", "replicationType"]

    @field_validator('replication_type')
    def replication_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['periodic']):
            raise ValueError("must be one of enum values ('periodic')")
        return value
