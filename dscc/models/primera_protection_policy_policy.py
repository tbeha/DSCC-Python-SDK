...
class PrimeraProtectionPolicyPolicy(BaseModel):
    """
    ...
    non_zero_rto_config: Optional[StrictStr] = Field(default=None, description="Non-Zero RTO configuration. Supported config is Active-Sync. Even the plain synchronous replication is non zero RTO configuration but for this plain sync configuration, no value will be specified in this parameter.", alias="nonZeroRtoConfig")
    over_period_alert: Optional[StrictBool] = Field(default=None, description="This field is valid for 2DC remote replication only, for 3DC remote replication check value of overPeriodAlert within remote and secondaryRemote. If synchronization of an asynchronous periodic Remote Copy group takes longer to complete than its synchronization period, an alert is generated. This property is not valid in case of synchronous policy and will always be false in that case.", alias="overPeriodAlert")
    remote: Optional[PrimeraRemoteInfo] = Field(default=None, description="Replication partner details")
    ...
    secondary_remote: Optional[PrimeraRemoteInfo] = Field(default=None, description="Second replication partner details from Synchronous Long Distance configuration and for 3DC Peer Persistence mode", alias="secondaryRemote")
    ...
    __properties: ClassVar[List[str]] = ["autoRecover", "autoSynchronize", "isProtectionPolicyConfigured", "noAutomaticSynchronization", "nonZeroRtoConfig", "overPeriodAlert", "remote", "rpoSecs", "secondaryRemote", "zeroRtoConfig"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "remote": PrimeraRemoteInfo.from_dict(obj["remote"]) if obj.get("remote") is not None else None,
            "rpoSecs": obj.get("rpoSecs"),
            "secondaryRemote": PrimeraRemoteInfo.from_dict(obj["secondaryRemote"]) if obj.get("secondaryRemote") is not None else None,
